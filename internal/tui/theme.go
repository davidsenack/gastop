...
// Color tag helper functions for cleaner code
func colorTag(color, text string) string {
	return "[" + color + "]" + text + "[-]"
}

func successTag(text string) string {
	return colorTag(currentTags.Success, text)
}

func warningTag(text string) string {
	return colorTag(currentTags.Warning, text)
}

func errorTag(text string) string {
	return colorTag(currentTags.Error, text)
}

func infoTag(text string) string {
	return colorTag(currentTags.Info, text)
}

func mutedTag(text string) string {
	return colorTag(currentTags.Muted, text)
}

func accentTag(text string) string {
	return colorTag(currentTags.Accent1, text)
}

func dimTag(text string) string {
	return colorTag(currentTags.Dim, text)
}

func boldTag(text string) string {
	return "[::b]" + text + "[::-]"
}
